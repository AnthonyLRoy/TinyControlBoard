...
esp_err_t McpInputHandler::begin(gpio_num_t sda, gpio_num_t scl, gpio_num_t intPin) {
    ...
    ESP_RETURN_ON_ERROR(initI2cBus(sda, scl), kLogTag, "I2C init failed");
    ESP_RETURN_ON_ERROR(initMcp23018(), kLogTag, "MCP init failed");
...
void McpInputHandler::enableI2c(bool enable) {
    ...
    ESP_LOGI(kLogTag, "I2C %s", enable ? "enabled" : "disabled");
}
...
void McpInputHandler::dumpRegisters() const {
    for (uint8_t reg = 0x00; reg <= 0x15; ++reg) {
        ...
        ESP_LOGI(kLogTag, "Reg 0x%02X = 0x%02X", reg, val);
    }
}
...
void McpInputHandler::scanI2c() const {
    ...
    for (uint8_t addr = 0x03; addr < 0x78; ++addr) {
        ...
        if (ret == ESP_OK) {
            ESP_LOGI(kLogTag, "Device at 0x%02X", addr);
            ++found;
        }
    }

    if (!found) {
        ESP_LOGW(kLogTag, "No I2C devices found");
    }
}
...
esp_err_t McpInputHandler::initI2cBus(gpio_num_t sda, gpio_num_t scl) {
    ...
    ESP_RETURN_ON_ERROR(i2c_param_config(mI2cPort, &conf), kLogTag, "I2C config failed");
    ESP_RETURN_ON_ERROR(i2c_driver_install(mI2cPort, I2C_MODE_MASTER, 0, 0, 0), kLogTag, "I2C install failed");
...
esp_err_t McpInputHandler::initInterruptPin() {
    ...
    ESP_RETURN_ON_ERROR(gpio_config(&io_conf), kLogTag, "Interrupt pin config failed");
...
void McpInputHandler::handleInterrupt() {
    ...
    ESP_LOGI(kLogTag, "Handling MCP interrupt");
    ESP_LOGI(kLogTag, "INTFA=0x%02X INTFB=0x%02X INTCAPA=0x%02X INTCAPB=0x%02X", intfA, intfB, intcapA, intcapB);
...
void McpInputHandler::decodeRotary(uint16_t state) {
    ESP_LOGI(kLogTag, "Decoding rotary with state: 0x%04X", state);
